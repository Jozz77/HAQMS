...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    // Fetch appointments with related Patient and Doctor in a single query (avoid N+1)
  ...
  try {
    ...
    if (error.code === 'P2002') {
      return res.status(400).json({
        error: 'Double booking blocked. Doctor already has an appointment at this exact slot.',
      });
    }
